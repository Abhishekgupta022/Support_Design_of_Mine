...
if support_j == 1:
    ...
    def calculate_bolt_length(t3, tm, crmr):
        global final_l
        if 20 <= crmr <= 80:
            bolt_length_binawiski = t3 / 3
            if tm == 1:
                bishop_bol_length = pow(t3, (2 / 3))
                final_l = (bolt_length_binawiski + bishop_bol_length) / 2
            elif tm == 2:
                bol_len_unal = (t3 * ((100 - crmr) / 100)) / 2
                final_l = (bolt_length_binawiski + bol_len_unal) / 2
        else:
            final_l = pow(t3, (2 / 3)) if tm == 1 else (t3 * ((100 - crmr) / 100)) / 2

        return round(final_l, 3)

    final_l = calculate_bolt_length(t3, tm, crmr)
    print("***************---HENCE---WE--GOT---****************")
    print(f"crmr {crmr}")
    print(f"you select: {tm}")
    print("Factor of Safety at Gallery is {}".format(format(fos_g, "0.2f")))
    print("Number of Bolts Required At Gallery is {}".format(b_g))
    print("Factor of Safety at Junction is {}".format(format(fos_j, "0.2f")))
    print("Number of Bolts Required At Junction is {}X{} is {}".format(b, b, pow(b, 2)))
...
pdf_filename = f"{mine_name}_report" + ".pdf"
